...
export function packageRecommendation(
  ...
  const compatible = prices.filter(item =>
    ...
    && Number(item.capacity) >= Number(guests)
  ...
  for (const option of compatible) {
    ...
    const prefix = seen.size ? 'Outra opção' : 'Opção premium (maior valor cadastrado)';
    const line = `\n• ${prefix}: *${displayText(option.name, 120)}* — ${money(option.price)}; capacidade de ${option.capacity} hóspedes.`;
