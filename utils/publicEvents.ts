...
export function publicEventInquiry(message: string): boolean {
  ...
  const publicContext = /\b(hoje|amanha|restaurante|reserva solar|hotel)\b/.test(s);
  ...
  return /\bheraldo(?:\s+ramos)?\b|\bmusica\s+ao\s+vivo\b|\bshows?\b|\bapresentacao\s+(?:musical|do musico|do cantor)\b/.test(s)
    || /\bprogramacao\b/.test(s) && /\b(restaurante|reserva solar|hotel|musical)\b/.test(s)
    || /\beventos?\b/.test(s) && /\b(hoje|amanha)\b/.test(s)
...
export function publicEventAnswer(message: string, now = Date.now()): string {
  ...
  if (otherHoliday(s) || new RegExp(`\\b(${MONTHS.filter(month => month !== 'setembro').join('|')})\\b`).test(s) && !/\b\d{1,2}\s+(?:de\s+)?(?:janeiro|fevereiro|marco|abril|maio|junho|julho|agosto|outubro|novembro|dezembro)\b/.test(s)) {
    return 'Não tenho uma apresentação de Heraldo Ramos confirmada para esse período. A programação informada é específica para 5 e 6 de setembro de 2026, no restaurante Reserva Solar. A equipe pode confirmar por aqui se há novidades para o período que você deseja.';
  }
  const today = localClock(now).date;
  const tomorrow = addDay(today, 1);
  const requested = requestedDates(s, today);
  const events = requested.length ? EVENTS.filter(event => requested.includes(event.data)) : [...EVENTS];
  ...
  if (missing.length) blocks.push(`Não tenho apresentação confirmada para ${missing.slice(0, 3).map(dateLabel).join(' ou ')}${missing.length > 3 ? ' e as demais datas solicitadas' : ''} na programação disponível.`);
