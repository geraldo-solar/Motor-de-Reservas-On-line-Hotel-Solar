        ...
        {(currentView === ViewState.HOME || currentView === ViewState.ROOMS || currentView === ViewState.PACKAGES) && (
          <div className="animate-in fade-in duration-1000">
            ...
            <section id="julho-section" className="max-w-7xl mx-auto px-4 pb-24 scroll-mt-24">
              <div className="relative rounded-[3rem] overflow-hidden bg-gradient-to-br from-solar-green via-solar-summer-blue to-solar-summer-teal p-12 md:p-20 shadow-2xl border border-white/10">
                <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]"></div>

                {/* Sol Decorativo de Fundo */}
                <div className="absolute -top-20 -right-20 w-80 h-80 bg-solar-summer-sun/20 rounded-full blur-3xl animate-pulse"></div>
                <div className="absolute top-1/2 -left-20 w-64 h-64 bg-solar-summer-blue/30 rounded-full blur-3xl"></div>
                  ...
                  {[
                    {
                      ...
                      checkIn: [2026, 6, 2],
                      checkOut: [2026, 6, 5],
                      img: '/celebracao-solar-original.jpg',
                      discount: '15',
                    ...
                    <div key={item.id} className={`group flex flex-col md:flex-row rounded-[2.5rem] overflow-hidden shadow-2xl border transition-all hover:-translate-y-1 ${item.isPromotional ? 'bg-orange-50/50 border-orange-200 shadow-orange-900/10' : 'bg-white border-white/10 shadow-solar-gold/20'}`}>
                      ...
                      <div className="md:w-3/5 p-8 md:p-12 flex flex-col justify-center space-y-8">
                        <div className="space-y-4">
                          ...
                          <div className={`inline-flex items-center gap-3 px-6 py-2.5 rounded-full shadow-lg transform -rotate-1 group-hover:rotate-0 transition-transform duration-500 ${item.isPromotional ? 'bg-solar-summer-orange text-white' : 'bg-solar-summer-sun text-solar-green'}`}>
                          ...
                          <div className="space-y-2">
                            {item.isPromotional && <span className="text-[10px] font-black text-solar-summer-orange uppercase tracking-[0.3em]">Melhor Valor de Julho</span>}
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {(item.programming || item.includes || []).map((prog: string) => (
                              <span key={prog} className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 border ${item.isPromotional ? 'bg-solar-summer-orange/10 border-solar-summer-orange/20 text-solar-summer-orange' : 'bg-slate-50 border-slate-100 text-slate-600'}`}>
                                <div className={`w-1.5 h-1.5 rounded-full ${item.isPromotional ? 'bg-solar-summer-orange' : 'bg-solar-summer-sun'}`}></div>
                                {prog}
                              </span>
                            ))}
                          </div>
                        ...
                        {!item.weeks && item.id !== 'jul-family' && (
                          <div className="pt-6 flex flex-col md:flex-row items-center gap-6">
                            <button
                              ...
                              className={`w-full md:w-auto px-10 py-5 text-white rounded-2xl font-bold uppercase text-xs tracking-[0.2em] transition-all shadow-xl active:scale-95 cursor-pointer z-30 ${item.isPromotional ? 'bg-solar-summer-orange hover:bg-solar-summer-orange/90 shadow-solar-summer-orange/20' : 'bg-solar-green hover:bg-solar-gold shadow-solar-green/20'}`}
                            >
                              Ver Disponibilidade
                            </button>
