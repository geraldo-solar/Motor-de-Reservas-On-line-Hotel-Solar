...
// Auto-restore Varanda Térreo if missing (Maintenance)
useEffect(() => {
  if (rooms.length > 0 && !isSaving) {
    const varanda = INITIAL_ROOMS.find(r => r.id === 'varanda-terreo');
    const exists = rooms.find(r => r.id === 'varanda-terreo');
    if (varanda && !exists) {
      console.log("Restaurando quarto 'varanda-terreo' para o banco de dados...");
      upsertRoom(varanda);
    }
  }
