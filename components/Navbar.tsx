...
const Navbar: React.FC<NavbarProps> = ({ currentView, onNavigate }) => {
  ...
  const navItemClass = (view: ViewState) => 
    `cursor-pointer px-2 sm:px-3 md:px-4 py-2 text-xs sm:text-sm md:text-sm font-semibold transition-all tracking-tight sm:tracking-normal md:tracking-wide whitespace-nowrap border-b-2 ${
      currentView === view 
    ...
    <nav className="bg-solar-green shadow-lg border-b border-white/10 py-2 sm:py-3 md:py-4">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-4">
        <div className="flex flex-col items-center justify-center">
          ...
          <div className="flex items-center cursor-pointer mb-2 sm:mb-3 md:mb-5 group" onClick={() => onNavigate(ViewState.HOME)}>
            <img 
              src="/logo.png" 
              alt="Hotel Solar" 
          ...
          <div className="w-full">
            <div className="flex flex-wrap items-center justify-center gap-x-1 sm:gap-x-2 md:gap-x-3">
              ...
              <button 
                ...
                className={navItemClass(ViewState.ROOMS)}
              >
                QUARTOS
              </button>
              <button 
